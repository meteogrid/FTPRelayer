...
class aemet_rename(object):
    ...
    def __call__(self, path):
        ...
        if basename.startswith('10'):
            new_name = 'Algunos-SYNOPs-' + basename[4:6] + self.now().strftime('-%Y-%m-%d-') \
                    + basename[6:8] + '00.txt'
        elif basename.startswith('SYNOP'):
            new_name = 'S' + basename[5:6] + self.now().strftime('-%Y-%m-%d-') \
                + basename[6:8] + '00.txt'
        elif basename.startswith('W'):
            new_name = basename[0:8] + self.now().strftime('_%Y-%m-%d_%H-%M-%S') \
                + '.txt'
        elif basename.startswith('SYHOAU'):
            new_name = 'SHA' + self.now().strftime('-%Y-%m-%d-') \
                + basename[6:8] + '00.txt'
        else:
            new_name = basename
        if errors:
